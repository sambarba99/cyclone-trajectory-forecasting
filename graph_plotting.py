...
def plot_cyclone_history_and_future(*, track_history: np.ndarray, track_future: np.ndarray, track_forecast: np.ndarray = None,
	...
	if track_forecast is None:
		...
		lat_mae = mean_absolute_error(lat_future, lat_forecast)
		long_mae = mean_absolute_error(long_future, long_forecast)
	...
	if save_to_file:
		plt.savefig(f'forecast_outputs/model_v{VERSION_NUM}_forecast_{file_id}.png')
...
def plot_loss(history: History) -> None:
	...
	plt.savefig(f'loss_graphs/model_v{VERSION_NUM}_loss.png')
	plt.show()
