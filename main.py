...
def main() -> None:
	...
	assert isinstance(VERSION_NUM, int) and VERSION_NUM in range(1, 5), f'VERSION_NUM must be an int from 1-4 (got: {VERSION_NUM})'
	...
	4. Only keep cyclone dataframes with enough recorded location history: since all model
	   versions use a certain history_size hours to forecast the next target_size hours, we
	...
	if choice == 'T':
		...
		model = load_model(f'saved_models/model_v{VERSION_NUM}.h5')
	...
	if choice == 'T':
		...
		print(f'\nView loss graph at /loss_graphs/model_v{VERSION_NUM}_loss.png.')
	...
	for idx, df in enumerate(test_cyclones, start=1):
		...
		y = vals[history_size:history_size + target_size]  # Must forecast the next target_size hours
	...
	print('View test set output forecasts in /forecast_outputs.')
	...
	if choice == 'T':
		...
		if choice == 'Y':
			model.save(f'saved_models/model_v{VERSION_NUM}.h5')
			print(f'\nSaved at /saved_models/model_v{VERSION_NUM}.h5.')
